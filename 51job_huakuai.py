    ...
    for keyword in keywords:
        ...
        for page in range(total_page):
            if valid_jobs>50:
                ...
                filename = f"./result/招聘分析结果_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            ...
            try:
                ...
                for i in range(num_jobs_on_page):
                    if valid_jobs>50:
                        df = pd.DataFrame(all_results)
                        filename = f"招聘分析结果_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                        df.to_excel(filename, index=False, engine='openpyxl')
                        print(f"结果已保存到 {filename}")
                        break
                    try:
                        ...
                        print(f"已处理职位ID: {job_id-1}, 学科: {related_discipline}")
